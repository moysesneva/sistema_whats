...
$sql_busca_usuario = "SELECT *
                      FROM login 
                      WHERE login = '$login'";
$query_busca_usuario = mysqli_query($conn, $sql_busca_usuario);

if ($query_busca_usuario) {
    $dados_usuario = mysqli_fetch_assoc($query_busca_usuario);

    // Atribui os valores dos campos às variáveis
    $solicitarConfirmacao = $dados_usuario['solicitar_confirmacao'];
    $mensagemConfirmacao = $dados_usuario['agenda_confirma'];
    $agendaVerifica = $dados_usuario['agenda_verfica'];
    $tempoVerifica = $dados_usuario['tempo_verifica'];

    $mensagemCancelamento = $dados_usuario['agenda_cancela'];
} else {
