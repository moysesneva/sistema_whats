...
CREATE TABLE `datas_excluidas` ( `id` int(255) NOT NULL, `data_excluida` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL, `id_profissional` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL, `motivo` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL, `profissional` varchar(255) COLLATE utf8_unicode_ci NOT NULL, `usuario_api` varchar(255) COLLATE utf8_unicode_ci NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
 ...
 CREATE TRIGGER `lista_negra_after_insert` AFTER INSERT ON `lista_negra` FOR EACH ROW BEGIN INSERT INTO lista_negra_log (lista_negra_id, acao, dados_novos, usuario_api) VALUES (NEW.id, 'INSERT', JSON_OBJECT( 'nome', NEW.nome, 'telefone', NEW.telefone, 'motivo_bloqueio', NEW.motivo_bloqueio, 'status', NEW.status ), NEW.usuario_api);

 END ;

 CREATE TRIGGER `lista_negra_after_update` AFTER UPDATE ON `lista_negra` FOR EACH ROW BEGIN INSERT INTO lista_negra_log (lista_negra_id, acao, dados_anteriores, dados_novos, usuario_api) VALUES (NEW.id, 'UPDATE', JSON_OBJECT( 'nome', OLD.nome, 'telefone', OLD.telefone, 'motivo_bloqueio', OLD.motivo_bloqueio, 'status', OLD.status ), JSON_OBJECT( 'nome', NEW.nome, 'telefone', NEW.telefone, 'motivo_bloqueio', NEW.motivo_bloqueio, 'status', NEW.status ), NEW.usuario_api);
