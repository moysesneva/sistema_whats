...
if ($dados) {
    ...
    if ($result_update) {
        ...
        echo "<p style='font-family:sans-serif'>Usuário <b>" . htmlspecialchars($usuario) . "</b> agora pode fazer login diretamente sem código.</p>";
    ...
    echo "<h2 style='color:orange;font-family:sans-serif'>⚠️ Usuário não encontrado: " . htmlspecialchars($usuario) . "</h2>";
    ...
    while ($row = mysqli_fetch_assoc($todos)) {
        echo "<tr><td>" . htmlspecialchars($row['login']) . "</td><td>" . htmlspecialchars($row['autorizado']) . "</td><td>" . htmlspecialchars($row['code_autorizado']) . "</td></tr>";
    }
    echo "</table>";
}
