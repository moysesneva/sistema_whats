...
$cs_telefone = '(+55) 31 8476-7330';
$cs_endereco = 'Belo Horizonte, MG - Brasil';

$res = mysqli_query($conn, "SELECT telefone, endereco FROM config LIMIT 1");
if ($res && $row = mysqli_fetch_assoc($res)) {
    if (!empty($row['telefone'])) {
        $cs_telefone = $row['telefone'];
    }
    if (!empty($row['endereco'])) {
        $cs_endereco = $row['endereco'];
    }
}
?>
<!DOCTYPE html>
